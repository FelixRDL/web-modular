...
/**
 * Class utilizes verbatim portions of spheras/knob (https://github.com/spheras/knob)
 */
...
export class KnobComponent implements ControlValueAccessor {
  ...
  /** the current rotation persisted (not persisted until the touch/mouse down is released) */
    ...
    ngOnInit() {
        var self = this;
        this.knobDiv = this.element.nativeElement.querySelector(".knob");
        this.knobDiv?.addEventListener('mousedown', function (e: any) { self.setChangeListener(e); });
        this.knobDiv?.addEventListener('touchstart', function (e: any) { self.setChangeListener(e); });
    ...
    private calculateDialAngle() {
        ...
        if(this.knobDiv) {
          this.knobDiv.style.transform = 'rotate(' + (rot) + 'deg)';
        }
      this.notifyChange();
    }
    ...
    calculateChange(mousePositionChange: number) {
        ...
        if(this.knobDiv) {
          this.knobDiv.style.transform = 'rotate(' + (newValue + this.startDegree) + 'deg)';
        }
